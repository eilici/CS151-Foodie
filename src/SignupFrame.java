		...
		txtSignUp_1.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getSource() == txtSignUp_1) {
					...
					try {
						p.checkPassword(new String(txtPassword.getPassword()));
					...
					User newUser = new Spartan(txtUsername.getText(), new String(txtPassword.getPassword()),
							new Preferences[] {});
					users.add(newUser);
					//we would also add this user info to the txt file;
